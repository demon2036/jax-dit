...
class ViTLayer(ViTBase, nn.Module):
    def setup(self):
        ...
        self.norm1 = nn.LayerNorm(epsilon=self.eps, use_fast_variance=self.use_fast_variance)
        self.norm2 = nn.LayerNorm(epsilon=self.eps, use_fast_variance=self.use_fast_variance)
...
class ViT(ViTBase, nn.Module):
    def setup(self):
        ...
        self.norm = nn.LayerNorm(epsilon=self.eps, use_fast_variance=self.use_fast_variance)
        self.head = Dense(self.labels) if self.labels is not None else None

        self.norm_pre=nn.LayerNorm(epsilon=self.eps, use_fast_variance=self.use_fast_variance)
    ...
    def forward_features(self, x: Array, det: bool = True) -> Array:
        # x = (x - IMAGENET_DEFAULT_MEAN) / IMAGENET_DEFAULT_STD
    ...
    def forward_head(self, x: Array, det: bool = True) -> Array:
        # x = (x - IMAGENET_DEFAULT_MEAN) / IMAGENET_DEFAULT_STD
        if self.pooling == "cls":
            x = x[:, 0, :]
        elif self.pooling == "gap":
