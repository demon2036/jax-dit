...
def create_state():
    ...
    state = TrainState.create(apply_fn=diffusion.apply, params=params, tx=tx, ema_params=params,
                              class_token_drop_key=jax.random.PRNGKey(1 + jax.process_index()),
                              times_key=jax.random.PRNGKey(11 + jax.process_index()),
                              gaussian_key=jax.random.PRNGKey(7 + jax.process_index()),
                              noise_key=jax.random.PRNGKey(2036 + jax.process_index()))
...
def train():
    ...
    b, h, w, c = 4, 32, 32, 4
    rng = jax.random.PRNGKey(444 + +jax.process_index())
    z = jax.random.normal(key=rng, shape=(b, h, w, c))
    model_kwargs = dict()

    rng = shard_prng_key(rng)
    z = shard(z)
    model_kwargs = shard(model_kwargs)
    p_sample_loop = functools.partial(diffusion_sample.p_sample_loop, shape=z.shape, clip_denoised=False, )

    @partial(jax.pmap, axis_name='batch')
    def sample_fn(params, noise, model_kwargs, key, vae_params):
        latent = p_sample_loop(params, noise=noise, model_kwargs=model_kwargs, key=key)
        latent = latent / 0.18215
        image = vae.apply({'params': vae_params}, latent, method=vae.decode).sample
        return image
    ...
    for i in range(5000):
        with tqdm.tqdm(dataloader) as pbar:
            ...
            if (i + 1) % 100 == 0:
                ...
                # latent = sample(state.ema_params['model'], noise=z, model_kwargs=model_kwargs, key=rng)
                img = sample_fn(state.ema_params['model'], noise=z, model_kwargs=model_kwargs, key=rng,
                                vae_params=vae_params)
                # img = vae_decode_image(latent, vae_params)

                img = process_allgather(img)

                if len(img.shape) == 6:
                    img = einops.rearrange(img, 'g p b c h w ->(g p b) c h w ')
                else:
                    img = einops.rearrange(img, 'p b c h w ->(p b) c h w ')
                show_image(img, i)
                end = time.time()
                print(end - start)

                # latent = sample_fn(diffusion_sample, state.ema_params['model'], )
...
def sample_fn(diffusion_sample, params, ):
    ...
    z = shard(z)
    model_kwargs = shard(model_kwargs)
    sample = jax.pmap(functools.partial(diffusion_sample.p_sample_loop, shape=z.shape, clip_denoised=False, ))

    # model_kwargs = dict(y=y)
    latent = sample(params, noise=z, model_kwargs=model_kwargs, key=rng)
    end = time.time()

    print(latent.shape, end - start)
